...
export const Poker: React.FC<Props> = ({ onClose }) => {
  const { authService } = useContext(AuthProvider.Context);
  const { gameService } = useContext(Context);
  const minigames = useSelector(gameService, _minigames);
  const minigame = minigames.games["poker"];
  const prize = minigames.prizes["poker"];
  ...
  if (isPlaying) {
    return (
      <div>
        <Portal portalName="poker" onClose={onClose} />
      </div>
    );
  }

  const onClaim = () => {
    gameService.send("minigame.prizeClaimed", { id: "poker" });
    onClose();
  };
  ...
  if (isComplete && !dailyAttempt.prizeClaimedAt && prize) {
    return (
      <ClaimReward
        onClaim={onClaim}
        reward={{
          message: "Congratulations, you won at poker! Here is your reward.",
          factionPoints: 0,
          id: "poker-reward",
          items: prize.items,
    ...
    <>
      <div className="mb-1">
        ...
        <MinigamePrizeUI
          ...
          mission={`Mission: Win ${prize?.score} chips`}
        />
      </div>
      <div className="flex">
