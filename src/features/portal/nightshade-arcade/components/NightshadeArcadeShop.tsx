...
import type { PortalMachineState, MachineInterpreter } from "../lib/nightshadeArcadePortalMachine";
...
const TIER_LABELS: Record<Tier, { label: string; type: "default" | "info" | "vibrant" | "warning" | "danger" }> = {
...
export const NightshadeArcadeShop: React.FC<NightshadeArcadeShopProps> = ({
  ...
  const ravenCoinBalance = typeof state.inventory.RavenCoin === "number" 
      ...
      <OuterPanel>
        ...
        <div className="flex items-center justify-end gap-2 mb-3 pb-2 border-b-2 border-gray-300 flex-shrink-0">
          <div className="flex items-center gap-1 bg-yellow-100 px-2 py-1 rounded border-2 border-yellow-700">
            <span className="text-2xl font-bold text-yellow-900 leading-none">{ravenCoinBalance}</span>
            <img src={RavenCoinIcon} alt="RavenCoin" style={{ width: "25px", height: "25px" }} />
          </div>
        </div>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="text-xs font-bold text-yellow-700 bg-yellow-100 px-1 rounded mt-auto flex items-center gap-1">
                          {cost}
                          <img src={RavenCoinIcon} alt="RavenCoin" style={{ width: "16px", height: "16px" }} />
                        </div>
                      </button>
                    );
                  })}
          ...
          <div className="bg-white rounded-lg border-4 border-gray-400 p-5 max-w-sm w-96 shadow-lg">
            ...
            <div className="mb-4 flex justify-center bg-gray-100 p-4 rounded">
              {ITEM_DETAILS[selectedItem.name as InventoryItemName]?.image ? (
                <img
                  src={ITEM_DETAILS[selectedItem.name as InventoryItemName]?.image}
            ...
            <div className="mb-4 space-y-2 bg-gray-50 p-3 rounded">
              <p className="text-sm">
                ...
                <span className="ml-2 text-yellow-700 font-bold flex items-center gap-1 inline-flex">
                  ...
                  <img src={RavenCoinIcon} alt="RavenCoin" style={{ width: "20px", height: "20px" }} />
                </span>
              </p>
              {selectedItem.cooldownMs && (
                <p className="text-xs text-gray-600">
                  <strong>Limited Item:</strong> Once per {Math.round(selectedItem.cooldownMs / (24 * 60 * 60 * 1000))} day(s)
                </p>
              )}
            </div>
              ...
              <button
                onClick={() => {
                  ...
                  if (ravenCoinBalance >= cost) {
                    ...
                    let tier: Tier = "basic";
                    for (const t of (["basic", "rare", "epic", "mega"] as const)) {
                      if (NIGHTSHADE_ARCADE_SHOP[t].items.some((i: any) => 
                        i.name === itemName
                      )) {
                        tier = t;
                      }
                ...
                disabled={ravenCoinBalance < (selectedItem.cost.items.RavenCoin || 0)}
